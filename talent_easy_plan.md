...
- Critical hit system (severity table, extra dice, speed reset, auto-ToP, instant kill).
- Near-Perfect/Perfect Defense counters (including crit tie-breaker).
- Improved Critical / Critical Mastery / Ranged Critical Mastery (weapon group).
- Wounding Criticals (weapon group).
- Knockback size adjustments (racial size overrides: small/large steps).

## Next easiest after review
1) Edge Counter (Katlakehan)
   - When perfect defense triggers, force the counter-attack to be a critical hit.

2) Defiant (Fymblwngen)
   - On critical hits against the defender, roll base damage twice and take the lower result.
   - Apply before crit severity is computed.

3) Superior Defense (Rogue)
   - Near-perfect defense triggers on natural 18 or 19.
   - Counter-blow damage uses 2d4p (no -2) instead of the default unarmed formula.

4) Armored to the Teeth (Pather)
...
6) Lithe + Inheritor (saving throw bonuses)
...
7) Stout (misc) + Sturdy (Vorova male)
   - Treat as one size larger for knockbacks by increasing `knockback_step` from 10->15 or 15->20.
   - Implement as a talent/racial modifier layered on top of race size.
